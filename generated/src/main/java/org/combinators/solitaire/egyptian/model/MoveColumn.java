...
public class MoveColumn extends Move {
    ...
    public boolean valid(Solitaire game) {
        return (!(source.empty()) && (destination.empty() || ((destination.peek().getRank() == (movingCards.peek(0).getRank() % 13) + 1 && destination.peek().oppositeColor(movingCards.peek(0))) && movingCards.alternatingColors())));
    }
}
