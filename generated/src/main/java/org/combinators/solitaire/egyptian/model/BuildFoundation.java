...
public class BuildFoundation extends Move {
    ...
    public boolean valid(Solitaire game) {
        return (!(source.empty()) && ConstraintHelper.ifCompute(destination.empty(), () -> (movingCards.count() == 1 && movingCards.peek(0).getRank() == Card.ACE), () -> ((movingCards.count() == 1 && movingCards.peek(0).getRank() == (destination.peek().getRank() % 13) + 1) && movingCards.peek(0).getSuit() == destination.peek().getSuit())));
    }
}
